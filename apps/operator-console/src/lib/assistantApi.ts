...
interface AssistantTextToSpeechOptions {
  signal?: AbortSignal;
}
...
function isAssistantTextToSpeechResponse(value: unknown): value is AssistantTextToSpeechResponse {
  ...
  return (
    (candidate.provider === 'kokoro' || candidate.provider === 'piper') &&
...
export async function synthesizeAssistantSpeech(
  text: string,
  options: AssistantTextToSpeechOptions = {}
): Promise<AssistantTextToSpeechResponse | null> {
