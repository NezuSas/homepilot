...
export const AutomationsLoadingState: React.FC<AutomationsLoadingStateProps> = ({ label }) => {
  return (
    <div className="flex-1 flex flex-col items-center justify-center min-h-empty-sm">
      <div className="premium-shimmer w-32 h-32 rounded-full absolute opacity-20" />
      <Loader2 className="w-10 h-10 animate-spin text-primary/40" />
      <p className="mt-6 text-micro font-black uppercase tracking-label-wider text-muted-foreground opacity-30">{label}</p>
    </div>
  );
};
