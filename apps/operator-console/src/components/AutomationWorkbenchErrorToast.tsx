...
export const AutomationWorkbenchErrorToast: React.FC<AutomationWorkbenchErrorToastProps> = ({ title, message, onRetry }) => {
  ...
  return (
    <div className="fixed bottom-[calc(env(safe-area-inset-bottom)+1rem)] right-3 z-[100] flex w-[calc(100%-1.5rem)] max-w-toast-responsive items-center gap-3 rounded-panel border border-danger/20 bg-danger p-4 text-danger-foreground shadow-depth-3 animate-in slide-in-from-right-4 backdrop-blur-md sm:bottom-6 sm:right-6 sm:w-auto sm:min-w-80 sm:gap-4 sm:p-5">
      ...
      <IconButton icon={RefreshCw} label={t('common.retry')} onClick={onRetry} variant="ghost" size="md" className="shrink-0 bg-white/20 text-danger-foreground hover:bg-white/40" />
    </div>
  );
};
