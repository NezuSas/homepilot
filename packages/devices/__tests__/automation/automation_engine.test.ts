...
describe('Automation Engine: Reactive Execution', () => {
  ...
  beforeEach(() => {
    ...
    // Inyección consistente según nuevo constructor
  ...
  it('debe garantizar aislamiento de fallos: el log AUTOMATION_FAILED debe incluir el nombre de la regla', async () => {
    await setupData();
    // Regla diseñada para fallar (el dispositivo no existe)
    await ruleRepo.save({
    ...
    // La regla válida ('rule-1') debe seguir funcionando
    expect(dispatcherMock.dispatch).toHaveBeenCalledWith('light-1', 'turn_on');
    
    // El log de fallo debe ser centralizado por el Engine e incluir el rule.name
