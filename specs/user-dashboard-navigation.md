...
20. La tarjeta `cover` del tablero debe reutilizar `CurtainDeviceTile`; su preview debe usar la misma estructura visual no interactiva, incluidos el nombre y la estancia del dispositivo seleccionado. El preview no debe presentar controles secundarios inactivos como si fueran operables y debe reservar la altura completa de la variante de cortina elegida, sin recortar la acción ni la posición. En tablero y Gestor de Dispositivos, una cortina única conserva un ancho máximo legible en vez de estirarse a todo el contenedor. Para el tamaño de tres tarjetas por fila, la composición de cortina se reduce a icono, estado, nombre y acción primaria; no debe comprimir ni recortar los controles secundarios.
