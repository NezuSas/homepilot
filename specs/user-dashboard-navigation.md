...
20. La tarjeta `cover` del tablero debe reutilizar `CurtainDeviceTile`; su preview debe usar la misma estructura visual no interactiva. En tablero y Gestor de Dispositivos, una cortina única conserva un ancho máximo legible en vez de estirarse a todo el contenedor. Para el tamaño de tres tarjetas por fila, la composición de cortina se reduce a icono, estado, nombre y acción primaria; no debe comprimir ni recortar los controles secundarios.
...
- **AC36:** Una cortina en el tablero muestra el control reutilizado del Gestor de Dispositivos, ejecuta comandos contra el inventario local y no supera el ancho máximo legible en escritorio si es la única unidad. El preview del editor conserva la misma jerarquía visual, sin controles activos. Al usar tres tarjetas por fila, tanto el preview como la tarjeta final emplean la variante compacta y mantienen una acción primaria legible sin recortes.
