...
11.1. Cuando la entidad multimedia importada publique una portada local (`entity_picture_local`) o remota (`entity_picture`), HomePilot debe priorizar la ruta local y mostrarla en la tarjeta mediante un proxy autenticado. Los tokens efímeros incluidos por Home Assistant en `entity_picture_local` no pueden persistirse ni exponerse al navegador: HomePilot debe obtener la imagen con el token administrativo del bridge. El preview debe reutilizar exactamente la misma tarjeta sin recortar sus controles.
12. En modo edición, el placeholder para crear una nueva zona debe colocarse en una fila completa después de la zona más baja del tablero; no puede compartir ni superponerse a una fila de zonas existente, incluso cuando una zona contiene tarjetas altas como cámaras, relojes o reproductores multimedia. La posición debe calcularse desde el contenido actual y no desde alturas heredadas de secciones antiguas.
