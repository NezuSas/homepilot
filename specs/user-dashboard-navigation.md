...
11.1. Cuando la entidad multimedia importada publique una portada local (`entity_picture_local`) o remota (`entity_picture`), HomePilot debe priorizar la ruta local y mostrarla en la tarjeta mediante un proxy autenticado, sin exponer el token de Home Assistant al navegador. El preview debe reutilizar exactamente la misma tarjeta sin recortar sus controles.
