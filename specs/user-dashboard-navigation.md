...
20. La tarjeta `cover` del tablero debe reutilizar `CurtainDeviceTile`; su preview debe usar la misma estructura visual no interactiva, incluidos el nombre y la estancia del dispositivo seleccionado. El preview debe permanecer no interactivo incluso cuando haya una cortina seleccionada, no debe presentar controles secundarios inactivos como si fueran operables y debe reservar la altura completa de la variante elegida, sin recortar la acción ni la posición. La variante media debe conservar un ancho visual menor que la completa. En tablero y Gestor de Dispositivos, una cortina única conserva un ancho máximo legible en vez de estirarse a todo el contenedor. Para el tamaño de tres tarjetas por fila, la composición de cortina se reduce a icono, estado, nombre y acción primaria; no debe comprimir ni recortar los controles secundarios. Cuando una sección reparte tarjetas en contenedores menores a 12rem, la cortina conserva icono, estado, nombre y acción primaria; oculta posición y controles secundarios para no truncarlos.

21. La tarjeta de cortina debe representar desde el inventario local de HomePilot los atributos sincronizados de Home Assistant que apliquen a una entidad `cover`: estado, posición actual, inclinación actual cuando exista y clase de dispositivo. La inversión de posición solo se aplica cuando la configuración local de ese dispositivo la declara; los controles se limitan a las capacidades importadas.
...
- **AC38:** Una tarjeta de sensor angosta no deja insignias vacías, etiquetas truncadas ni estados duplicados; una lectura porcentual sitúa su valor y título en una jerarquía vertical legible, con un único estado compacto y sin desbordamiento horizontal.
- **AC39:** El shell y los widgets de tablero aplican una jerarquía residencial sobria: el encabezado y las pestañas se separan por líneas discretas, el contenido conserva un ancho legible y las superficies informativas reducen sombras y capas decorativas sin alterar las acciones ni contratos existentes.
- **AC40:** La tarjeta multimedia conserva la portada ya cargada ante actualizaciones de estado que no cambian su fuente; solo renueva la sesión y descarga de portada cuando Home Assistant publica una ruta de imagen distinta.
