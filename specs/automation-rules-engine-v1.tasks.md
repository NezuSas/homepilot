...
- [x] **T01: Definir `AutomationRule` en `domain/automation/types.ts`**
...
- [x] **T02: Definir `AutomationRuleRepository` en `domain/repositories/`**
  - Interfaz contractual con métodos: `save`, `findById`, `findByTriggerDevice`, `findByHomeId`, `delete`.
- [x] **T03: Definir nuevos errores de negocio en `domain/errors.ts`**
  - Sugeridos: `AutomationLoopError`, `CrossHomeAutomationError`, `AutomationRuleNotFoundError`.

### Lógica Pura
- [x] **T04: Factoría de Reglas con Validación en `domain/automation/AutomationRule.ts`**
...
- [x] **T05: Implementar `InMemoryAutomationRuleRepository`**
...
### Gestión (CRUD)
- [x] **T06: Implementar `CreateAutomationRuleUseCase`**
...
- [x] **T07: Implementar `ListAutomationRulesUseCase`**
  - Filtrar por `homeId` y validar ownership.
- [x] **T08: Implementar `DeleteAutomationRuleUseCase`**
  - Validar ownership antes de eliminar.

### Motor de Evaluación (Engine)
- [x] **T09: Implementar `AutomationEngine` (Suscriptor de Eventos)**
...
- [x] **T10: Integrar Contexto de Seguridad e Identidad**
  - Asegurar que `AutomationEngine` recupere el `userId` de la regla para el chequeo de ownership en la ejecución.
- [x] **T11: Implementar Logging de Auditoría Especial con Prefijos**
...
- [x] **T14: Pruebas de Dominio (`automation_domain.test.ts`)**
  - Validar prevención de bucles y tipos de datos del trigger.
- [x] **T15: Pruebas de Casos de Uso CRUD (`automation_crud.test.ts`)**
  - Validar AC1 y AC4 (Zero-Trust).
- [x] **T16: Pruebas del Motor (`automation_engine.test.ts`)**
  - Simular `DeviceStateUpdatedEvent`.
  - Verificar AC2 (ejecución correcta) y AC3 (registro de fallo ante incompatibilidad).

### End-to-End (E2E)
- [x] **T17: Escenarios E2E en `automation_e2e.test.ts`**
...
- [x] **AC1**: Creación válida persiste la regla vinculando dispositivos del mismo hogar.
- [x] **AC2**: Se dispara el comando automáticamente al detectar el cambio de estado disparador.
