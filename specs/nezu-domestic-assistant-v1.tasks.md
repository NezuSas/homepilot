...
- [x] Cover deterministic named-room status (including unique partial room references and readable scoped state groups), room-qualified cover commands, unavailable-device, and global active-light queries without weakening confirmation. Evidence: `assistant_conversation_service.test.ts`.
