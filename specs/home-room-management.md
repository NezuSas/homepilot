...
- [x] **AC10**: Dado un administrador propietario, al enviar `PATCH /api/v1/rooms/:id` con un nombre válido, el sistema retorna la Room actualizada, incrementa `entityVersion` y mantiene intactas las asignaciones por `roomId`. Evidencia: `packages/topology/__tests__/application.test.ts`.
- [x] **AC11**: Dado un usuario que no es propietario, al intentar renombrar una Room el sistema retorna `403 Forbidden` sin modificarla. Evidencia: `packages/topology/__tests__/application.test.ts`.
...
- [x] **AC24**: Las luces de una Room mantienen nombre, estado y control en columnas estables. Las etiquetas Encendido y Apagado se muestran como metadato secundario de 10 px, son compactas, legibles y no se desbordan ni cambian de posición cuando varía el nombre del dispositivo.
